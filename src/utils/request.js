...
import { randomKey, aesEncrypt, getRsaKey, rsaEncrypt, styleEncrypt, aesDecrypt } from '@/utils/encrypt'
import { getData, setData, getToken } from '@/utils/auth'

// 随机生成key
const initKey = randomKey(true, 10, 32)
function encryptResponse(responseData) {
    // 解析请求体，获取rsa公钥
    const pubkey = getRsaKey(responseData)
    // 存储rsa公钥
    store.dispatch('setChange', { name: 'PUBKEY', value: pubkey }).then(() => {
        setData('PUBKEY', pubkey)
    })
    // 解析请求体，获取version
    const tempData = styleEncrypt(responseData + '')
    responseData = JSON.parse(tempData)
    // 存储version
    store.dispatch('setChange', { name: 'VERSION', value: responseData.version }).then(() => {
        setData('VERSION', responseData.version)
    })
    return responseData
}
...
service.interceptors.request.use(config => {
    ...
    if (getData('PUBKEY')) {
        // 请求头进行rsa加密
        const rsaData = rsaEncrypt(initKey, getData('PUBKEY'))
        ...
        config.headers['pubkey_version'] = getData('VERSION')
    } else {
        config.headers['pubkey_version'] = '1970-1-1 00:00:00'
    }
    config.headers['sjzencode'] = 1
    // 请求体进行ase加密, 调用ase加密函数加密请求体
    config.data = aesEncrypt(JSON.stringify(config.data), initKey)
    return config
}, () => {
...
service.interceptors.response.use(
    response => {
        if (String(response.data).indexOf('pubKey') > 0) {
            ...
            // 有公钥的前提下对返回体解密
            response.data = aesDecrypt(response.data, store.getters.aeskey)
        ...
        if (res.returnCode !== 0) {
            ...
            } else if (getToken() && res.returnCode === 555) {
